...
class edge {
	...
	finalize() {
		...
		this.print()
		console.log("- finalizing this edge")
		...
		this.node2.setdepth(this.realize_word().length + this.node1.getdepth())
	}
	...
	split(pointer) {
		assert(!this.abandoned)
		/*console.log(this.realize_word(), pointer, (this.node1.getpedge() && this.node1.getpedge().abandoned))
		console.log(this.abandoned)
		console.log(this.node2.getpedge() === this, this.node2.getpedge())
		console.log(this.node2.getpedge() && this.node2.getpedge().abandoned|| "")*/
		/* (n1)---------------->(n2) 
				|
				|
				v
		...
		*/
		...
		let w2 = [w[0] + pointer, w[1]] // w[1] may be "#" but we dont care
		...
		*/
		...
		let edge_to_nu_leaf = new edge(split_node, nu_leaf, [i, "#"])
	...
	print() {
		...
		console.log(this.node1.getn(), this.node2.getn(), this.realize_word())
	}
}
...
class node {
	...
	isinternal() {
		return this.parent && this.kids.length > 0
	}
	...
	replace_kid(old, nu) {
		...
		let old_edge_name = old.getpedge().realize_word()
		let nu_edge_name = nu.getpedge().realize_word()
		
		assert(old_edge_name[0] === nu_edge_name[0])
		
		this.fltredges[old_edge_name[0]] = nu.getpedge()
	}
	...
	addkid(edge, kid) {
		...
		assert(!this.fltredges[edge.realize_word()[0]], edge.realize_word(), kid.getn())
		
		this.fltredges[edge.realize_word()[0]] = edge
	}
	...
	getkidbyname(name) {
		return this.namekids[name]
	}
	
	getkidbyedge(name) {
		return this.fltredges[name[0]].getn2()
	}
	...
	print_compact(tab) {
		...
		for(let k in this.kids) {
			console.log(tabs + "", this.kids[k].edge.realize_word_for_print(), ">")
			this.kids[k].kid.print_compact(tab + 1)
		}
		
		if(this.suffixlinkto) {
			console.log(tabs + "->", this.suffixlinkto.getn())
		}
		
		if(this.suffixlinkfrom) {
			console.log(tabs + this.suffixlinkfrom.getn(), "->")
		}
	...
	rosa_print(res) {
		if (!res) res = ""
		for(let k in this.kids) {
			let e = this.kids[k].edge
			if((res.indexOf("\n$\n") !== -1 || res.substr(0,2) === "$\n") && e.realize_word() === "$")
				continue
			
			res += e.realize_word() + "\n"
			res = this.kids[k].kid.rosa_print(res)
		}
	...
	edges_print(res) {
		...
		for(let k in this.kids) {
			...
			res += e.realize_word() + "\n"
			res = this.kids[k].kid.edges_print(res)
		}
...
class active_point {
	...
	adjust_active_edge(offs) {
		...
		let w = e.realize_word()
		console.log("Adjustment for rule1, w.length =", w.length, "active_length =", this.active_length)
		let peek = this.sq.peekword()
		
		if (w.length < len) {
			console.log("Dangerous!!!")
			
			assert(this.active_length <= peek.length, [this.active_length, peek.length])
			assert(w === peek.substr(offs, w.length), [w, peek, peek.substr(offs, w.length), offs, i])
			
			let dif = len - w.length
			this.active_length = dif
			console.log(peek, peek.length, w, w.length, this.active_length, offs, dif)
			this.active_node = e.getn2()
			this.active_edge = peek[offs + w.length]
			
			this.adjust_active_edge(offs + w.length)
	...
	decrement_rule1() {
		...
		/*if(this.active_length === 0 && this.active_node.fltredges[peek[0]]) {
			let e = this.active_node.fltredges[peek[0]]
			let peekl = peek.length
			this.active_edge = peek[0]
			console.log(peekl)
			this.active_length = peekl
		}*/
		
		// if active_edge is finalized, flip may be needed
		...
		if(this.active_length >= w.length) {
	...
	follow_suffix_link_or_set_to_root_rule3() {
		...
		let w = this.find_active_edge() && this.find_active_edge().realize_word()
	...
	generate_from_active_node() {
		...
		let edg = new edge(this.active_node, leaf, [i, "#"])
		leaf.sete(edg)
	...
	is_ch_already_present_at_active_node() {
		return this.active_node.findedge(ch) != void 0 || this.active_length !== 0
	}
	...
	is_goes_along_active_edge() {
		...
		//console.log("\t...(active_word, pos, char_at_pos, cur_char)=(", w, l, w[l], ch, ")")
	...
	split_active_edge() {
		...
		this.root.print_compact()
		//console.log(res, Object.keys(res), res.nu_leaf, "split node")
		//console.log(res.split_node)
...
let bst = (s) => {	
	...
	let root = new node(void 0, "root", void 0)
	...
	console.log(root.print_compact())
	return root
}
...
ATAAATG$.realize()
console.log(JSON.stringify(ATAAATG$.json(), null, 2))
...
console.log(tree.edges_print())
let out = tree.edges_print()
